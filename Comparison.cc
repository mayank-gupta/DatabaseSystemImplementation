...
int CNF :: get_query_ordermaker(OrderMaker *query_ordermaker,OrderMaker* literal_query_ordermaker,OrderMaker* sortOrder){
  for(int k=0;k<sortOrder->numAtts;k++){
    int whichAtt = sortOrder->whichAtts[k];
    Type whichType = sortOrder->whichTypes[k];
    bool found = false;
	...
	continue;
      ...
      if((  (orList[i][0].operand1!=Literal) && orList[i][0].operand2== Literal  ) && orList[i][0].attType==whichType){
	if(orList[i][0].whichAtt1==whichAtt){
	  //match
	  //add it to the query ordermaker
	  query_ordermaker->whichAtts[query_ordermaker->numAtts] = whichAtt;
	  query_ordermaker->whichTypes[query_ordermaker->numAtts] = whichType;
	  query_ordermaker->numAtts++;
	  literal_query_ordermaker->whichAtts[literal_query_ordermaker->numAtts] = orList[i][0].whichAtt2;
	  literal_query_ordermaker->whichTypes[literal_query_ordermaker->numAtts] = whichType;
	  literal_query_ordermaker->numAtts++;
	  found = true;
	  break;
	}
      }

      if((  (orList[i][0].operand2!=Literal) && orList[i][0].operand1==Literal) && orList[i][0].attType==whichType){
	if(orList[i][0].whichAtt2==whichAtt){
	  //match
	  //add it to the query ordermaker
	  query_ordermaker->whichAtts[query_ordermaker->numAtts] = whichAtt;
	  query_ordermaker->whichTypes[query_ordermaker->numAtts] = whichType;
	  query_ordermaker->numAtts++;
	  literal_query_ordermaker->whichAtts[literal_query_ordermaker->numAtts] = orList[i][0].whichAtt1;
	  literal_query_ordermaker->whichTypes[literal_query_ordermaker->numAtts] = whichType;
	  literal_query_ordermaker->numAtts++;
	  found=true;
	  break;
    ...
    //not found so no more looking up
    if(found==false){
      return query_ordermaker->numAtts;
    }
  }
